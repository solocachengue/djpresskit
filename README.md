...
- **Los tres slots de la tapa** — se superponen en el centro, así que en modo edición cada uno se anuncia con su nombre y su hint queda anclado en distinta altura: **fondo a sangre** (foto gigante detrás de todo), **marca de fondo** (logo grande detrás del wordmark) y **prop chico** (objeto sobre el wordmark). Un slot con imagen muestra su nombre en una etiqueta arriba a la izquierda, porque una vez cargado ya no se ve el hint y no habría forma de saber cuál estás por reemplazar.
- **La marca de fondo** — el slot *marca de fondo* pone un logo enorme detrás del wordmark. La app mide el borde de la imagen y elige sola el modo de fusión: un logo blanco sobre negro se compone con `screen` y uno negro sobre blanco con `multiply`, así el plato opaco de la imagen desaparece en vez de estampar un recuadro. Arranca al 50% de brillo; se sube con `◐`.
